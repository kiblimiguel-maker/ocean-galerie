...
export default function AdminPage() {
  ...
  // Einfacher Schutz: Nur deine E-Mail hat Zugriff
  const isAdmin = user && user.email === "mxocean@protonmail.com";

  if (loading) return <div className="min-h-screen bg-ocean-darkest flex items-center justify-center text-ocean-gold">Lädt...</div>;
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-ocean-darkest flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-ocean-muted mb-8">Diese Seite ist nur für den Administrator (Artemis) zugänglich.</p>
        <a href="/" className="text-ocean-gold border border-ocean-gold/30 px-6 py-2 rounded-full hover:bg-ocean-gold hover:text-ocean-darkest transition-all">Zurück zur Galerie</a>
      </div>
    );
  }
              ...
              <div key={art.id} className="bg-ocean-card border border-ocean-border/40 rounded-xl p-6 md:p-8 flex flex-col md:flex-row gap-8 items-start md:items-center">
                ...
                <div className="w-full md:w-64 bg-ocean-darkest/50 rounded-lg p-4 border border-ocean-border/30">
                  <p className="text-[10px] text-ocean-muted uppercase tracking-widest mb-3 flex items-center gap-2">
                    <User size={10} /> {ended ? "Gewinner" : "Höchstbietender"}
                  </p>
                  {art.lastBidder ? (
                    <div>
                      ...
                      <p className="text-[10px] text-ocean-muted italic mt-2">
                        {ended ? "👉 Kontaktiere den Gewinner jetzt." : "Auktion läuft noch..."}
                      </p>
                    </div>
                  ) : (
